...
class SLEIPNIR_DLLEXPORT VariableMatrix {
 public:
    ...
    for (int i = 0; i < lhs.rows(); ++i) {
      for (int j = 0; j < rhs.Cols(); ++j) {
        ...
        for (int k = 0; k < lhs.cols(); ++k) {
          sum += MakeConstant(lhs(i, k)) * rhs(k, j);
        }
        result(i, j) = sum;
      }
    }
  ...
  friend VariableMatrix operator*(
    ...
    for (int i = 0; i < lhs.Rows(); ++i) {
      for (int j = 0; j < rhs.cols(); ++j) {
        Variable sum = 0.0;
        for (int k = 0; k < lhs.Cols(); ++k) {
          sum += lhs(i, k) * MakeConstant(rhs(k, j));
        }
        result(i, j) = sum;
      }
    }
  ...
  friend VariableMatrix operator+(
    ...
    for (int row = 0; row < result.Rows(); ++row) {
      for (int col = 0; col < result.Cols(); ++col) {
        result(row, col) = MakeConstant(lhs(row, col)) + rhs(row, col);
      }
    }
  ...
  friend VariableMatrix operator+(
    ...
    for (int row = 0; row < result.Rows(); ++row) {
      for (int col = 0; col < result.Cols(); ++col) {
        result(row, col) = lhs(row, col) + MakeConstant(rhs(row, col));
      }
    }
