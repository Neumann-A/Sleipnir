...
Eigen::VectorXd ProjectedCG(Eigen::SparseMatrix<double> G, 
                                 Eigen::VectorXd c, 
    ...
    for (int iteration = 0; iteration < /*2 * (x.rows() - A.rows())*/ 200; ++iteration) {
      double tmp1 = r.dot(g);
...
//   std::cout << "A eigenvalues: \n" << Eigen::SimplicialLDLT<Eigen::SparseMatrix<double>>{G}.vectorD() << std::endl;
